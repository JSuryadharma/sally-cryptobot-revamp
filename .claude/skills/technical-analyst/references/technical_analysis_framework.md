...
robocrypto's own EMA-cross strategy (`src/decisionEngine.js` `STRATEGY_PARAMS.swing`) trades EMA20/EMA50 on *daily* candles. Using the same two EMAs on the *weekly* chart keeps this manual read directly comparable to what the bot's own `swing` mode is watching, one timeframe up:

- **EMA20 (weekly)**: short/medium-term trend indicator (~20 weeks, ~4-5 months)
- **EMA50 (weekly)**: medium/long-term trend indicator (~50 weeks, ~1 year)

(The original equity-oriented version of this framework also referenced a 200-week MA for a multi-year view; robocrypto's own indicator set - `src/indicators.js` - tops out at a 50-period EMA/SMA, so this rewrite keeps the comparison to EMA20/EMA50 exact rather than inventing an unused third line. Add a 200-week SMA yourself if a longer-horizon view is wanted for a specific chart - it's an EMA/SMA like any other, just not one the bot itself trades on.)

### Moving Average Interpretations

**Price Position Relative to MAs:**
- **Above both EMAs**: Strong bullish structure
- **Below both EMAs**: Strong bearish structure
- **Between EMAs**: Transitional phase, trend unclear

**Moving Average Crossovers:**
- **Golden Cross**: EMA20 crosses above EMA50 (bullish signal) - the exact event `evaluateEntry`'s `swing` mode fires a BUY on, at the daily timeframe; on the weekly chart it's the bigger-picture version of the same signal
- **Death Cross**: EMA20 crosses below EMA50 (bearish signal) - `evaluateExit`'s `swing` mode fires a trend-exit SELL on this, daily
...
- In uptrends, EMA20/EMA50 often provide dynamic support
- In downtrends, they often provide dynamic resistance
- Repeated bounces or rejections from an EMA increase its significance

### Moving Average Confluence

- **Bullish Alignment**: EMA20 > EMA50 (both rising)
- **Bearish Alignment**: EMA20 < EMA50 (both falling)
- **Compressed/Converging EMAs**: often precedes a significant directional move - and is exactly the condition under which `src/decisionEngine.js`'s own confidence formula scores lowest on the "EMA separation" component (see the `backtest-expert` skill and this session's fix to that formula), since a fresh cross by definition starts from near-zero separation
