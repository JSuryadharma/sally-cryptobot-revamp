...
| `GET api.coingecko.com/api/v3/coins/{id}/market_chart` | Daily close history (alts) | none |
| `GET api.coingecko.com/api/v3/global` | Current BTC dominance | none |
| `GET {settings.binanceBaseUrl}/api/v3/klines` | BTC daily close history | none |
| `GET fapi.binance.com/fapi/v1/premiumIndex` | Latest funding per perp | none |

Fetches are throttled at ~8s between CoinGecko history calls to respect its free-tier limits and cached per UTC day under `--cache-dir`.

**Deviation from the original Python skill**: BTC's own daily closes come from `data-api.binance.vision` (robocrypto's `src/config.js` `binanceBaseUrl`, the same endpoint `scripts/benchmark.mjs` uses) instead of a CoinGecko `market_chart` call - one fewer CoinGecko round trip, and it isn't geo-blocked the way `api.binance.com`/`fapi.binance.com` are on some networks. `fapi.binance.com` has no such vision-style mirror, so the funding component keeps hitting it directly and is allowed to fail closed (`data_available: false`) rather than crash - this was confirmed necessary while testing this port (funding was unreachable, and the rest of the report still generated correctly).
