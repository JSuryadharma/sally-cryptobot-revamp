...
The bands below are the original equity market-cap convention; robocrypto's own friction model is simpler and worth naming explicitly:

**robocrypto's actual friction**: `roundTripCostPct` (DB-backed, see `src/settings.js`) is applied as a single fixed round-trip cost on every simulated trade in `scripts/benchmark.mjs`'s `openPosition`/`closePosition` - it is never zero, but it is also never stress-tested at a pessimistic multiple by default. If validating a result, temporarily raise it (e.g. 1.5-2x) and re-run before trusting the baseline as a worst case.

**By market capitalization** (equity convention, useful context for BTC/ETH-scale liquidity vs. a thin altcoin):
...
- Stop orders: Significant slippage in volatile conditions (crypto flash-wicks make this worse than the equity case this table was written for)
...
**Volatility-based regimes (equity convention, VIX)**:
- Low volatility: VIX <15
- Normal volatility: VIX 15-25
- High volatility: VIX 25-35
- Extreme volatility: VIX >35

**Crypto has no VIX equivalent traded by robocrypto** - use `.claude/skills/crypto-regime-analyzer`'s Drawdown & Volatility component instead (BTC realized 30d volatility vs its trailing 1y percentile) for the same kind of regime split.
...
**Issue**: Testing only on currently-trading stocks ignores delisted/bankrupt companies. The crypto equivalent: testing only on today's Binance watchlist ignores tokens Binance has delisted or that went to zero - `scripts/benchmark.mjs` only ever fetches symbols you hand it today, so a watchlist survivorship bias is easy to introduce without noticing.

**Solution**: Use survivorship-bias-free datasets that include historical delistings, or explicitly note in any report that the tested symbol set is "coins currently on the watchlist," not "every coin that ever met the criteria."
